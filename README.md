<div align="center">
  <img src="docs/img/sos-vault-logo.png" alt="sos-vault" width="480" />
...
| <img src="docs/img/screenshot-ai-assistant.png" width="380" alt="Mil AI Assistant"><br>**9. Ask Mil** — the built-in AI Assistant answers questions about the loaded case, grounded in the actual report data. | <img src="docs/img/screenshot-compliance.png" width="380" alt="Compliance & Exposure"><br>**10. Compliance & Exposure** — CIS, STIG, and Docker Bench findings assessed automatically on every upload, with remediation guidance for each failing check. |
| <img src="docs/img/screenshot-alerts.png" width="380" alt="Alerts"><br>**11. Alerts** — rules (file-pattern match, metric threshold, or reference-case diff) evaluated automatically on every new upload, with in-app, email, and event-log delivery. | |
...
| Alert rules (file-pattern / threshold / diff) | ✅ (Slack requires license) | Team & Enterprise plans | ✅ (Slack requires license) |
| Compliance & Exposure (CIS / STIG / Docker Bench) | ✅ | ✅ | ✅ |
...
### Compliance & Exposure
- **CIS, STIG, and Docker Bench assessment** — every uploaded sosreport is automatically checked against CIS, STIG (DISA-sourced rule text), and Docker Bench benchmarks, no configuration required
- **Exposure scoring** — a network/firewall, software, identity, and systemd exposure assessment runs alongside the compliance benchmarks
- **Score pills + findings table** — a per-ruleset pass-rate summary and a filterable findings table with remediation guidance for every failing check

### Alerting
- **Alert rules** — file-pattern match, metric threshold, or reference-case diff rules evaluated automatically on every new sosreport upload
- **Multi-channel delivery** — in-app notification, email, and event log / SIEM forwarding on every install; Slack contact-point delivery on the licensed self-hosted tier and SaaS Team/Enterprise plans
- **Vault-wide or case-scoped** — manage every rule from the vault-wide Alerts page, or create one directly from a file in the sos Viewer
