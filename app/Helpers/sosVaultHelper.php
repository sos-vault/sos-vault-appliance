...
function addEvent($payload, $type, $status, $class, $cid, $vid, $owner, $group)
{
    ...
    TYPE:
        ...
        // Alerts (grep / threshold / reference-case diff rules)
        'ALERT_GRP':      "Grep alert rule matched class=NORMAL",
        'ALERT_THRESH':   "Threshold (levels) alert rule matched class=NORMAL",
        'ALERT_DIFF':     "Reference-case diff alert rule matched class=NORMAL",

        // Compliance & Exposure Engine (CIS/STIG/Docker Bench/Exposure analysis runs)
        'COMPLIANCE_RUN':      "Compliance analysis run started/completed/failed class=NORMAL",
        'COMPLIANCE_FINDING':  "HIGH/CRITICAL compliance finding(s) detected class=NORMAL",
...
/**
 * Best-effort: raise the PHP memory_limit for operations that unpack/process
 * large sosreports (VaultTools, SosServiceProvider). Skipped when it can't
 * help — the limit is already unlimited or already at/above the target —
 * since ini_set() can only raise the ceiling, never set it below memory
 * already in use. In a long-lived process (e.g. a queue worker or a full
 * PHPUnit run constructing many of these back to back) usage only grows, so
 * once one caller raises the limit, every later one is a no-op here instead
 * of re-attempting a "raise" to the same value after usage has since climbed
 * past it — which previously threw ("failed to set memory limit … current
 * usage is …", converted to an ErrorException by Laravel's error handler)
 * and crashed unrelated requests under memory pressure.
 */
function raiseMemoryLimit(string $target): void
{
    $targetBytes = memoryLimitToBytes($target);
    $currentLimitBytes = memoryLimitToBytes((string) ini_get('memory_limit'));

    if ($currentLimitBytes === -1 || $currentLimitBytes >= $targetBytes) {
        return;
    }

    if (memory_get_usage(true) >= $targetBytes) {
        return;
    }
...
/** Parse a php.ini memory_limit-style value ("512M", "-1", "1073741824") to bytes. */
function memoryLimitToBytes(string $value): int
{
    $value = trim($value);
    if ($value === '-1' || $value === '') {
        return -1;
