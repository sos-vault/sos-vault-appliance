...
class DataTools extends ServiceProvider
{
    ...
    // List the immediate (non-directory) entry names under a directory path in
    // the report tree — e.g. drop-in dirs like etc/ssh/sshd_config.d that the
    // compliance fact extractor needs to enumerate rather than read directly.
    // Missing directory or no children just means an empty list, never an error.
    public function listDirEntries($dirPath): array
    {
        $dirInfo = explode('/', trim($dirPath, '/'));
        $dirName = array_pop($dirInfo);
        $path = $dirInfo ? implode('/', $dirInfo).'/' : '';

        $found = $this->vtools->find_node_by_attr($this->tree->nodes, 'name', $dirName, 'path', $path);
        if (! $found || empty($found->nodes)) {
            return [];
        }
    ...
    /**
     * Best-effort parse of the sosreport docker plugin capture (docker_info /
     * docker_-D_info + etc/docker/daemon.json). Most sosreports carry no
     * docker plugin data at all — that's the common case, not an error — so
     * this returns null rather than throwing when nothing is found.
     */
    public function getDockerData()
    {
        $jsonContents = "{$this->path}/.dockerData.json";

        $cached = $this->cached;
        if ($cached && is_file($jsonContents)) {
            $dockerData = json_decode(file_get_contents($jsonContents), true);
            if (json_last_error() == JSON_ERROR_NONE) {
                return $dockerData;
            }
        }
        ...
        $infoFiles = [
            'sos_commands/docker/docker_info',
            'sos_commands/docker/docker_-D_info',
        ];
        ...
        $daemonConfig = null;
        if (is_file("{$this->path}/etc/docker/daemon.json")) {
            $daemonLines = $this->readFileContents('etc/docker/daemon.json');
            if ($daemonLines) {
                $decoded = json_decode(implode("\n", $daemonLines), true);
                if (json_last_error() == JSON_ERROR_NONE) {
                    $daemonConfig = $decoded;
                }
            }
        }

        if (empty($contents) && $daemonConfig === null) {
            return null;
        }
        ...
        // docker_info is indentation-structured (Client:/Server: blocks); the
        // fields we care about live directly under Server: (indent 1), and
        // "Security Options:" is followed by further-indented sub-items.
        $fieldMap = [
        ...
        if ($contents) {
            foreach ($contents as $line) {
                ...
                if ($inSecurityOptions && $indent > $securityIndent) {
                    // one level deeper than "Security Options:" itself is an option
                    // name (apparmor, seccomp, ...) — these are bare, not "key: value";
                    // anything deeper still (e.g. a "Profile: builtin" sub-line under
                    // seccomp) is skipped, not captured as its own option
                    if ($indent === $securityIndent + 1) {
                        $data['security_options'][] = str_contains($trimmed, ':')
                            ? trim(explode(':', $trimmed, 2)[0])
                            : $trimmed;
                    }
