...
class VaultTools extends ServiceProvider
{
    ...
    public function parseFilename($filename)
    {
        ...
        if (isset($nameparts)) {
            ...
            // A filename edited after being generated by sos report (extra segments
            // inserted/removed, e.g. an instance-type suffix tacked onto the name)
            // desyncs this positional parsing: the year/month/day/sosid slots no
            // longer line up with a real date or a plausible id. Reject rather than
            // persist a garbage date, which later crashes Carbon-backed table columns.
            if (! $this->isPlausibleReportDate($year, $month, $day) || ! preg_match('/^[A-Za-z0-9]+$/', (string) $fdata->sosid)) {
                throw new InvalidSosreportFilenameException(
                    "Could not parse the filename \"{$filename}\": the date and report id segments are not in the expected sos report format. The file may have been renamed after being generated by sos report."
                );
            }
    ...
    /**
     * Whether $year/$month/$day (as extracted positionally by parseFilename())
     * form a real calendar date. Guards against a renamed/malformed filename
     * shifting unrelated segments (e.g. an id or a suffix) into these slots.
     */
    private function isPlausibleReportDate($year, $month, $day): bool
    {
        return preg_match('/^\d{4}$/', (string) $year)
            && preg_match('/^\d{1,2}$/', (string) $month)
            && preg_match('/^\d{1,2}$/', (string) $day)
    ...
    public function unpack($key, $filename, $fileid)
    {
        ...
        try {
            ...
            Log::warning($message);
            notifyError($message);
            $this->user && notifyUser($this->user, $message, 'error');
        ...
        if ($decrypted) {
            ...
            if (isset($did)) {
                ...
                // Evaluate Alert rules in the background — never on the upload request path.
                AlertsEvaluationRequested::dispatch(auth()->user()->id, $vid, $did, $cid);

                // Run compliance analysis in the background — never on the upload request path.
                ComplianceAnalysisRequested::dispatch(auth()->user()->id, $vid, $did, $cid);
            }
    ...
    public function doDecryptAndExtract($filename, $dir, $pass, &$did, &$cid, &$errorMesg, $statusfile = null, $customer = null, $version = null, $link = null, $caseOverride = null, $selfHostedUserId = null)
    {
        ...
        try {
            ...
            Log::warning($errorMesg);
            notifyError($errorMesg);
            $this->user && notifyUser($this->user, $errorMesg, 'error');
            $this->emessage = $errorMesg;
