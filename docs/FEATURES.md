...
- **Alert rules.** File-pattern match, metric-threshold, and reference-case-diff rules evaluated automatically on every sosreport upload, with in-app, email, and event-log/SIEM delivery. Open on every appliance install regardless of license — only the optional Slack contact-point channel is gated (see below).
- **Compliance & Exposure Engine.** CIS, STIG, and Docker Bench compliance assessment plus a network/software/identity/systemd exposure check, run automatically on every sosreport upload. Fully unlicensed — available on every plan and every appliance install.
...
- **Alert rules — Slack delivery.** In-app, email, and event-log/SIEM delivery for Alert rules work without a license; the Slack contact-point channel requires one.
