...
function factory(win) {
    ...
    const banned = ["link", "script"];
    const required = ["nodeType", "nodeValue", "tagName"];
    const ignored = ["attributes", "childNodes", "children", "classList", "dataset", "style"];
    const serials = ["innerHTML", "innerText", "outerHTML", "outerText", "prefix", "text", "textContent", "wholeText"];
    ...
    const boolInter = function (item, filter) {
        let output;
        if (item instanceof Array) {
    ...
    const boolDiff = function (item, filter) {
        let output;
        if (item instanceof Array) {
            output = unique(item.filter(function (val) {
                return filter.indexOf(val) === -1;
    ...
    const boolFilter = function (item, filter) {
        if (filter === false) {
    ...
    const toShorthand = function (filterList) {
        let outputArray;
    ...
    const toAbsolute = function (value, origin) {
    ...
    const copyJSON = function (node, opts) {
        let copy = {};
        for (let n in node) {
    ...
    const attrJSON = function (node, opts) {
        let attributes = {};
    ...
    const styleJSON = function (node, opts) {
        let style, css = {};
    ...
    const toJSON = function (node, opts, depth) {
        let style, kids, kidCount, thisChild, children, copy = copyJSON(node, opts);
        if (node.nodeType === 1) {
            for (let b in banned) {
    ...
    const createNode = function (type, doc, data) {
    ...
    const toDOM = function (obj, parent, doc) {
        if (obj.nodeType) {
        ...
        for (let x in obj) {
            if (x !== "isContentEditable" && x !== "childNodes") {
                try {
                    node[x] = obj[x];
                } catch (e) {
                    continue;
