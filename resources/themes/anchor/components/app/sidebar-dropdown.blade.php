<div x-data="{ {{ $id }}: {{ $open ?? false }} }"
    ...
    >
        <div class="flex relative items-center w-full h-auto">
            <x-dynamic-component :component="$icon" x-data="{tooltip: '{!! $text !!}', get getTooltip() {const sdc = localStorage.getItem('sidebarColapsed'); return sdc ? this.tooltip : '' }}" x-tooltip.placement.right="getTooltip" class="shrink-0 mr-2 w-6 h-6" />
            <span class="sbcolapse mr-0">{{ $text }}</span>
            <span :class="{ 'rotate-180' : {{ $id }} == true }" class="sbcolapse mr-0.5 ml-auto w-4 h-4 duration-300 ease-out">
                <x-phosphor-caret-down class="w-full h-full" />
            </span>
        </div>

        <template x-teleport="#{{ $id }}">
            <div class="absolute z-80 p-1 space-y-1 bg-zinc-100 dark:bg-zinc-800" x-show="{{ $id }}" x-collapse x-cloak>
                {{ $slot }}
            </div>
        </template>
    </div>
