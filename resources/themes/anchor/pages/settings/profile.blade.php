...
{
    use HasDynamicFields, InteractsWithForms;
    ...
    public function save()
    {
        ...
        Notification::make()
            ->title(__('settings.profile_save_success'))
            ->success()
            ->send();
    }
    ...
    private function saveNewUserAvatar()
    {
        ...
        $image = Image::read($this->avatar)->resize(800, 800);
        ...
        class="relative w-full">
            <form wire:submit="save" class="w-full">
                <div class="relative flex flex-col mt-5 lg:px-10">
                    ...
                    @error('avatar')
                        <p class="mt-3 text-sm text-red-600">{{ __('settings.profile_avatar_invalid_type') }}</p>
                    @enderror
                    ...
                    <div class="flex items-center gap-4 mt-6">
                        <div>
                            <x-label for="language" value="{{ __('settings.language') }}" />
                            <p class="text-sm text-zinc-500 mt-1">{{ __('settings.language_description') }}</p>
                        </div>
                        <div class="flex gap-2 mt-2">
                            @foreach(config('app.supported_locales', ['en' => 'English']) as $code => $name)
                                <a href="{{ route('locale.switch', $code) }}"
                                   class="px-3 py-1.5 text-sm font-medium rounded-full border transition
                                          {{ app()->getLocale() === $code
                                              ? 'bg-zinc-900 text-white border-zinc-900 dark:bg-white dark:text-zinc-900'
                                              : 'bg-white text-zinc-700 border-zinc-200 hover:bg-zinc-50 dark:bg-zinc-800 dark:text-zinc-200 dark:border-zinc-700' }}">
                                    {{ $name }}
                                </a>
                            @endforeach
                        </div>
                ...
                <x-filament::modal id="profile-avatar-crop">
                    <div>
                        <div class="mt-3 text-center sm:mt-5">
                            <h3 class="text-lg font-medium leading-6 text-zinc-900" id="modal-headline">
                                {{ __('settings.profile_avatar_crop_heading') }}
                            </h3>
                            <div class="mt-2">
                    ...
                    <div class="mt-5 sm:mt-6">
                        <span class="flex w-full rounded-md shadow-sm">
                            <button @click="window.dispatchEvent(new CustomEvent('close-modal', { detail: { id: 'profile-avatar-crop' }}));" class="inline-flex justify-center w-full px-4 py-2 mr-2 text-base font-medium leading-6 transition duration-150 ease-in-out bg-white border border-transparent rounded-md shadow-sm text-zinc-700 border-zinc-300 hover:text-zinc-500 active:text-zinc-800 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue sm:text-sm sm:leading-5" type="button">{{ __('settings.cancel') }}</button>
                            <button @click="window.dispatchEvent(new CustomEvent('close-modal', { detail: { id: 'profile-avatar-crop' }})); applyImageCrop()" class="inline-flex justify-center w-full px-4 py-2 ml-2 text-base font-medium leading-6 text-white transition duration-150 ease-in-out bg-blue-600 border border-transparent rounded-md shadow-sm hover:bg-blue-500 focus:outline-none focus:border-blue-700 focus:shadow-outline-wave sm:text-sm sm:leading-5" id="apply-crop" type="button">{{ __('settings.profile_avatar_crop_apply') }}</button>
                        </span>
                    </div>
                </x-filament::modal>
            </div>
