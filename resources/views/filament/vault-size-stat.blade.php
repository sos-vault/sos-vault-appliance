@php
    ...
    // Progress-bar colour derived from usage percentage
    $pct      = (int) ($pct ?? 0);
    $barClass = match (true) {
        $pct >= 85 => 'bg-danger-500',
        $pct >= 80 => 'bg-warning-500',
...
>
    <div class="fi-wi-stats-overview-stat-content">
        ...
        {{-- Progress bar --}}
        <div class="mt-2 w-full rounded-full bg-gray-200 dark:bg-gray-700" style="height:6px;">
            <div
                class="rounded-full transition-all duration-500 {{ $barClass }}"
                style="width: {{ min($pct, 100) }}%; height:6px;"
                title="{{ $pct }}% used"
            ></div>
        </div>
    </div>
