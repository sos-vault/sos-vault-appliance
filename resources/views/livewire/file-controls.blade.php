...
new class extends Component implements HasActions, HasSchemas
{
    ...
    public bool $hasTriggeredAlert = false;

    public string $alertSeverity = '';

    public string $alertBadgeColor = 'gray';
    ...
    public function mount($caseid, $fid, $parent, $color, int $sme = 0)
    {
        ...
        if (isset($metadata)) {
            ...
            $trigger = AlertTrigger::where('case_id', $this->caseid)
                ->where('matched_path', ltrim($this->filepath, '/'))
                ->latest()
                ->with('alert')
                ->first();

            if ($trigger && $trigger->alert) {
                $this->hasTriggeredAlert = true;
                $this->alertSeverity = $trigger->alert->severity;
                $this->alertBadgeColor = match ($this->alertSeverity) {
                    'INFO' => 'gray',
                    'WARNING' => 'warning',
                    'ERROR' => 'orange',
                    default => 'danger', // CRITICAL, FATAL
                };
            }
        }
    ...
    public function getFileTools(): array
    {
        ...
        if (isAppliance() || checkAccess(auth()->user(), 'Alert Rules')) {
            $tools[] = Action::make('addAlert')
                ->icon('phosphor-bell-ringing-duotone')
                ->extraAttributes([
                    'class' => 'w-32',
                ])
                ...
                ->outlined()
                ->color('warning')
                ->label(__('alerts.file_add_alert_label'))
                ->tooltip(__('alerts.file_add_alert_tooltip'))
                ->action(function () {
                    $url = str_replace(
                        ['[vid]', '[did]', '[caseid]'],
                        [$this->vid, $this->did, $this->caseid],
                        '/sosTool/[vid]/[did]/Alerts/[caseid]'
                    ).'?create=1&path='.urlencode($this->filepath);
                    $this->dispatch('checkTab', ['url' => $url, 'tabName' => 'SOS Alerts']);
                });
        }
