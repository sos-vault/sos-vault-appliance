    ...
    new class extends Component implements HasSchemas, HasActions
    {
        ...
        public function getTools(): array
        {
            ...
            foreach(Tools::query()->where('showInMenu', true)->get() as $tool) {
                if($tool->url) {
                    // Per-tool feature-name overrides. Kept here (name-keyed) rather than
                    // as an extra column on Tools::getRows() — Sushi builds its in-memory
                    // schema from the array shape of the rows, so a key present on only
                    // one row breaks the generated INSERT for every other row.
                    $featureOverrides = [
                        'Alerts' => 'Alert Rules',
                    ];
                    // Unlicensed tools: free on both SaaS and appliance, regardless of
                    // plan or license state — bypass checkAccess()/appliance gating
                    // entirely rather than routing through a feature name.
                    $openCoreTools = ['Compliance'];
                    $featureName = $featureOverrides[$tool->name] ?? match($tool->type) {
                        'advanced' => 'Advanced Tools',
                        'special'  => 'Special Tools',
                        default    => 'Basic Tools',
                    };
                    // On appliance builds checkAccess() has no SaaS Plan to resolve for
                    // any role, so it always returns false — a per-tool feature gate
                    // (e.g. "Alert Rules") must be bypassed there rather than silently
                    // blocking every appliance user. Gating for such tools is handled
                    // elsewhere on appliance (e.g. per-channel licensing), not here.
                    $hasPlanAccess = in_array($tool->name, $openCoreTools, true)
                        || isAppliance()
                        || checkAccess(auth()->user(), $featureName);
