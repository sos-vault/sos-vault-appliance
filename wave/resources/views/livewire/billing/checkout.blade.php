            ...
            <div class="h-full space-y-5">
                @foreach($plans as $plan)
                    ...
                    <div
                        ...
                        <div class="flex flex-col mb-10 h-full bg-white dark:bg-neutral-800 rounded-xl ease-out duration-300 border-2 border-gray-200 dark:border-neutral-700 shadow-sm sm:mb-0 group-hover:border-{{ config('wave.billing_color') }}-500">
                                ...
                                <div class="flex items-center justify-end w-full">
                                    <div class="relative w-full md:w-auto">
                                        @if(config('wave.billing_provider') == 'stripe')
                                            <x-billing.button wire:click="redirectToStripeCheckout('{{ $plan->id }}')" wire:target="redirectToPaymentProvider" rounded="md" color="{{ config('wave.billing_color') }}">
                                        ...
                                        @else
                                                ...
                                                <x-filament::modal width="lg" id="change-plan-modal">
                                                    <x-slot name="trigger">
                                                            @if($plan->id == $userPlan->id)
                                                                <x-billing.button rounded="md" color="{{ config('wave.billing_color') }}" x-show="billing_cycle_selected == '{{ $userSubscription->cycle }}'">You are currently on this plan</x-billing.button>
                                                                <x-billing.button rounded="md" color="{{ config('wave.billing_color') }}" x-show="billing_cycle_selected != '{{ $userSubscription->cycle }}'">Switch to this plan</x-billing.button>
                                                            @else
                                                                <x-billing.button rounded="md" color="{{ config('wave.billing_color') }}">Switch to this plan</x-billing.button>
                                                            @endif
                                                    </x-slot>
                                                    <div x-data class="relative flex flex-col items-center justify-center">
                                                        <div
                                                            ...
                                                            <div class="flex items-center justify-center shrink-0 w-12 h-12 mx-auto bg-blue-100 rounded-full sm:mx-0 sm:h-10 sm:w-10">
                                            ...
                                            @else
                                                <x-billing.button x-on:click="
                                                    ...
                                                    "
                                                    rounded="md" color="{{ config('wave.billing_color') }}"
                                                >
                                                    Subscribe to this Plan
                                                </x-billing.button>
                                            @endif
