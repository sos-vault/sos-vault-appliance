...
class User extends Authenticatable implements FilamentUser, HasAvatar, JWTSubject, MustVerifyEmail
{
    ...
    public function daysLeftOnTrial(): int
    {
        if ($this->trial_ends_at && $this->trial_ends_at >= now()) {
            return (int) ceil(abs(Carbon::parse($this->trial_ends_at)->diffInDays(now())));
        }
    ...
    {
        $plan = Plan::whereEnglishName($planSlug)->first();
        if (! $plan) {
            return false;
        }
    ...
    public function invoices()
    {
        ...
        if (config('wave.billing_provider') == 'stripe') {
            ...
            $user_invoices = $responseJson->data;
    ...
    public function createApiKey($name, $userkey = null)
    {
        $key = (isset($userkey) && $userkey != '') ? $userkey : Str::random(60);
    ...
    /**
     * Return default User Role (legacy Voyager-compatible belongsTo via role_id column).
     */
